...
/*   Updated: 2023/09/18 15:36:50 by abiddane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_check_textures(t_struct *all, char **tab)
{
	...
	{
		all->data->no++;
		all->data->nimg = ft_strdup(tab[1]);
	}
	else if (!ft_strncmp(tab[0], "SO", 3))
	{
		all->data->so++;
		all->data->simg = ft_strdup(tab[1]);
	}
	else if (!ft_strncmp(tab[0], "WE", 3))
	{
		all->data->we++;
		all->data->wimg = ft_strdup(tab[1]);
	}
	else if (!ft_strncmp(tab[0], "EA", 3))
	{
		all->data->ea++;
		all->data->eimg = ft_strdup(tab[1]);
	}
	else
